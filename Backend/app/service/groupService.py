...
class GroupService:
    ...
    def _to_output(self, group) -> GroupOutput:
        ...
        """
        # שליפת member access levels ישירות מה-DB במקום מה-relationship
        member_access_levels = self.session.query(MemberGroupAccess).filter(
            MemberGroupAccess.group_uuid == group.UUID
        ).all()
        
        unique_member_ids = list(dict.fromkeys(m.member_uuid for m in member_access_levels))

        return GroupOutput(
            UUID=group.UUID,
            name=group.name,
            members=unique_member_ids,
            meetings=[],  # נשאיר ריק כרגע כי ה-meetings relationship מושבת
            member_access_levels=[
                MemberAccessOutput(
                    user_id=row.member_uuid, access_level=row.access_level
                )
                for row in member_access_levels
            ],
        )
    ...
    def get_all_groups(self) -> list[GroupOutput]:
        return [self._to_output(group) for group in self.__groupRepository.get_all_groups()]

    def get_groups_by_user_uuid(self, user_uuid: str) -> list[GroupOutput]:
        """×ž×—×–×™×¨ ×¨×§ ××ª ×”×ž×“×•×¨×™× ×©×œ ×”×ž×©×ª×ž×© ×”×ž×—×•×'×¨"""
        return [
            self._to_output(group)
            for group in self.__groupRepository.get_groups_by_user_uuid(user_uuid=user_uuid)
        ]
